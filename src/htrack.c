...
static inline void remapd3(double v[3], const double old_m[3], const double new_m[3], const double factor[3]) {
    v[0] = remapd(v[0], old_m[0], new_m[0], factor[0]);
    v[1] = remapd(v[1], old_m[1], new_m[1], factor[1]);
    v[2] = remapd(v[2], old_m[2], new_m[2], factor[2]);
}
...
static const double limits_in[3] = {75, 75, 90};
static const double limits_out[3] = {135, 90, 90};
static const double smooth_factor[3] = {1.5, 1.5, 1.5};
...
void htk_frame() {
    ...
    if(state.must_reset) {
        ...
        state.viewport_ref.x = XPLMGetDataf(state.dr.ref_x);
        state.viewport_ref.y = XPLMGetDataf(state.dr.ref_y);
        state.viewport_ref.z = XPLMGetDataf(state.dr.ref_z);
    }
    ...
    normd3(state.head + 3);
    remapd3(state.head + 3, limits_in, limits_out, smooth_factor);
    
    XPLMSetDataf(state.dr.head_x, -1e-2 * state.head[0] + state.viewport_ref.x);
    XPLMSetDataf(state.dr.head_y, 1e-2 * state.head[1] + state.viewport_ref.y);
    XPLMSetDataf(state.dr.head_z, 1e-2 * state.head[2] + state.viewport_ref.z);
